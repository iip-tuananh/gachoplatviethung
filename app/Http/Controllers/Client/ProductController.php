...
// return;
    ...
    public function getProductTabHome(Request $request)
    {
        $id = $request->id;
        $cate_id = $request->cate_id;
        $cate = Category::findOrFail($cate_id);
        $typeCate = TypeProduct::findOrFail($id);
        $data['products'] = Product::where(['status'=>1, 'discountStatus'=>1, 'type_slug'=>$typeCate->slug, 'cate_slug'=>$cate->slug])->get(['name','id','slug','cate_slug', 'type_slug','images','price','discount']);
        $html = view('layouts.product.view-tab-home', $data)->render();
